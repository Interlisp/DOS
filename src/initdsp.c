/* @(#) initdsp.c Version 1.2 (4/22/92). copyright venue & Fuji Xerox  */
static char *id = "@(#) initdsp.c	1.2 4/22/92	(venue & Fuji Xerox)";
...
#ifdef DOS
#define getpagesize() 4096	/* Sez' in the manual... */
#endif /* DOS */
...
#if (defined(XWINDOW) || defined(DOS))
    (currentdsp->enter_display)(currentdsp);
    displaywidth  = currentdsp->displaywidth;
    displayheight = currentdsp->displayheight;
#endif /* XWINDOW */
...
/*									*/
/************************************************************************/
long int old_color0, old_color1;
...
#if defined(XWINDOW) || defined(DOS)
    (currentdsp->exit_display)(currentdsp);
#endif /* DOS */
	}
...
flush_display_buffer()
  {
...
#ifdef XWINDOW
   (currentdsp->bitblt_to_screen)(currentdsp,
				  DisplayRegion68k,
				  currentdsp->UpperLeftX,
				  currentdsp->UpperLeftY,
   ...
   (currentdsp->bitblt_to_screen)(currentdsp,
				   DisplayRegion68k,
				   0, 0,
				   currentdsp->displaywidth,
				   currentdsp->displayheight );
  ...
  int bitoffset, ybase, w, h;
{
  ...
  pr_rop(ColorDisplayPixrect, x, y, w, h, COPY_PIXRECT_TO_COLOR,
	 DisplayRegionPixrect, x, y);
