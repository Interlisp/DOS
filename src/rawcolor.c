/* @(#) rawcolor.c Version 1.2 (4/22/92). copyright venue & Fuji Xerox  */
static char *id = "@(#) rawcolor.c	1.2 4/22/92	(venue & Fuji Xerox)";
...
C_slowbltchar(args)
  register LispPTR *args;
  {
	...
	cl_top = n_dd->ddclippingtop;
 	...
 	newx = curx + *(DLword*)Addr68k_from_LADDR(n_dd->ddwidthscache
						     + Char8Code(charcode));
	TPRINT((" 2 C_slowbltchar"));
	...
	/* I assume we can take care of this later /jarl ... */
	{  
	  WINDOW *window;
	  SCREEN *ColorScreenData;
	  if(COLORSCREEN_index==0xffffffff)
	    { /* Make sure COLOR lives? */
	  ...
	  if((displaystream != ColorScreenData->SCTOPW) &&
		(displaystream != window->DSP) &&
		(displaystream != *TOPWDS68k) && 
		((fmemb(n_dd->dddestination,*SCREENBITMAPS68k)) != NIL))
	  PUNT_TO_SLOWBLTCHAR; 
	}
#endif /* not XWINDOW */
	...
	{ register PILOTBBT *pbt;
	  ...
	  src_x =  *((DLword*)Addr68k_from_LADDR( n_dd->ddoffsetscache 
							+ Char8Code(charcode)));
	  src_y = 0;
	  w = *(DLword*)Addr68k_from_LADDR(n_dd->ddcharimagewidths 
							+ Char8Code(charcode));
	  h = src_h;

    TPRINT((" 4 C_slowbltchar"));
	  ...
	  if(GetTypeNumber( n_dd->dddestination ) == TYPE_BITMAP)
	    { /* Bitap */
	      ...
	      displayflg=n_new_cursorin(
	    	Addr68k_from_LADDR(n_destBM->bmbase),dst_x<<3,
	    		/* Kludge:YCoordination upside down*/
		    	displayheight-cury,w,h); 
	      if(displayflg) HideCursor;

    TPRINT((" 6 C_slowbltchar"));

	      ColorizeFont8(n_srcBM, src_x, src_y
				, n_destBM, dst_x, dst_y
				, w, h
				, backcolor,forecolor
	    ...
	    { /* BIGBM */
	      ...
	      ColorizeFont8_BIGBM(n_srcBM, src_x, src_y
				, n_destBIGBM, dst_x, dst_y
				, w, h
				, backcolor,forecolor
				, n_dd->ddsourcetype,n_dd->ddoperation);
	      ScreenLocked=NIL;
...
ColorizeFont8(sBM,sXOffset,sYOffset,dBM,dXOffset,
	      dYOffset,width,height,col0,col1,sourcetype,operation)
  register BITMAP *sBM;
...

{
 ...
 nbase=(DLword*)Addr68k_from_LADDR(sBM->bmbase)
		+ (sBM->bmrasterwidth * sYOffset);
 dbase= (DLword*)Addr68k_from_LADDR(dBM->bmbase)
		+ (dBM->bmrasterwidth * dYOffset);
 for(i=0, dbase += dXOffset;/* 8bpp */
     i < height;
     i++,nbase += sBM->bmrasterwidth , dbase += dBM->bmrasterwidth)
 {
...
ColorizeFont8_BIGBM(sBM,sXOffset,sYOffset,dBM,dXOffset,dYOffset,width,height,col0,col1,sourcetype,operation)
register BITMAP *sBM;
register DLword sXOffset,sYOffset,width,height;
register BIGBM *dBM;
...
/* height of lineBlt8 */
 ...
 dbase= (DLword*)Addr68k_from_LADDR(dest_frag->bmbase)
		+ (dest_frag->bmrasterwidth * dest_yoffset);
 nbase=(DLword*)Addr68k_from_LADDR(sBM->bmbase)
		+ (sBM->bmrasterwidth * sYOffset);

 sYOffset += (DLword)dest_h; /* next src yoffset */

 for(i=0, dbase += dXOffset;
     i < dest_h;
     i++, nbase += sBM->bmrasterwidth , dbase += dest_frag->bmrasterwidth)
 {
	lineBlt8(nbase,(int)sXOffset,dbase,(int)width,col0,col1,sourcetype,operation);
 }
  ...
  LispPTR srctype,ope;
{
    ...
    dbase= (u_char*)Addr68k_from_LADDR(
...
LispPTR args[];
{
	...
	d_base = (u_char*)Addr68k_from_LADDR( d_bitmap->bmbase )
		+ d_bitmap->bmrasterwidth * (d_bitmap->bmheight - (d_bottom + height));

	for( i=0, d_base += d_left; i<height; 
	     i++, s_base += s_bitmap->bmrasterwidth
	   , d_base += d_bitmap->bmrasterwidth ) {

		lineBlt8( s_base, s_left, d_base, width
			, (u_char)color0, (u_char)color1
			, INPUT_atom, REPLACE_atom );

	} /* end for(i) */
	TPRINT(("Exit Colorize_Bitmap\n"));
 bad_arg: return( NIL );
...
#define draw8bppliney(op)					\
	while( (x0 <= xlimit) && (y0 <= ylimit ) ) {		\
		*base op color; base += yinc; ++y0; cdl += dx;	\
		if( dy <= cdl ) {				\
			cdl -= dy;				\
			++ base; ++x0;				\
		}						\
...
Draw_8BppColorLine( args )
  register LispPTR *args;
  {
    ...
    yinc   = (short)( args[7] &0xffff ); yinc *= 2; /* for byte addressing */

    if     ( args[8] == PAINT_atom  ) mode = 3;
    else if( args[8] == INVERT_atom ) mode = 2;
    else if( args[8] == ERASE_atom  ) mode = 1;
    else 				  mode = 0; /* REPLACE_atom */
    ...
    if( yinc >= 0 )
      {
	displayflg = n_new_cursorin( (DLword*)n_bmbase
		, (int)(x0<<3), (int)y0, (int)(xlimit<<3), (int)ylimit );
      }
    else
      {
	displayflg =
	   n_new_cursorin( (DLword*)n_bmbase, 
			   (int)(x0<<3), (int)(y0-ylimit),
    ...
    if( dx >= dy )
      { /* .draw8bpplinex */
	switch( mode ) {
		case 0:	draw8bpplinex( op_replace );
			break;
		case 1: draw8bpplinex( op_erase );
			break;
		case 2: draw8bpplinex( op_invert );
			break;
		case 3: draw8bpplinex( op_paint );
			break;
	  } /* end switch */
      ...
      { /* .draw8bppliney */
	switch( mode ) {
		case 0: draw8bppliney( op_replace );
			break;
		case 1: draw8bppliney( op_erase );
			break;
		case 2: draw8bppliney( op_invert );
			break;
		case 3: draw8bppliney( op_paint );
			break;		
	  } /* end switch */
