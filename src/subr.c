/* @(#) subr.c Version 2.64 (4/23/92). copyright venue & Fuji Xerox  */
static char *id = "@(#) subr.c	2.64 4/23/92	(venue & Fuji Xerox)";
...
switch(Get_BYTE(PC+1))
  {
	...
	case sb_COLOR_INIT : POP_SUBR_ARGS;
			TopOfStack=make_dsp_instance(colordsp, args[0], 0, 0, 8);
	                (colordsp->enter_display)(colordsp);
			break;
	case sb_COLOR_SCREENMODE : POP_SUBR_ARGS;
			TopOfStack=SwitchDisplay(args[0]);
			break;
	case sb_COLOR_MAP : POP_SUBR_ARGS;
			TopOfStack=(currentdsp->set_color_map_entry)(args[0]);
			break;
...
#ifdef DOS
	  TopOfStack = DOS_handlecomm(args);
#else /* DOS */
