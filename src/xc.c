/* @(#) xc.c Version 1.30 (4/21/92). copyright venue & Fuji Xerox  */
static char *id = "@(#) xc.c    1.30 4/21/92    (venue & Fuji Xerox)";
...
register LispPTR tscache asm("bx"); /* DX not saved thru calls... */
#endif
...
nextopcode :
  ...
  OPTPRINT(("Dispatch, PC = 0x%x, op = 0%o. TOS = 0x%x, BITBLT def = 0x%x.\n", (int)PCMAC, Get_BYTE_PCMAC0, TOPOFSTACK, *(int *)(GetDEFCELL68k(6740))));
#ifdef MYOPTRACE
      }
#endif /* MYOPTRACE */
                                ...
                                !(((INTSTAT2 *)intstate)->handledmask & 0x40))
                              {
                                ...
                                if(period_cnt>0)  period_cnt--;
                                        ...
                                        if(*PERIODIC_INTERRUPT_FREQUENCY68k==NIL)
                                                ...
                                                period_cnt=(*PERIODIC_INTERRUPT_FREQUENCY68k & 0xffff) *(1000000/60) 
/TIMER_INTERVAL;
