/* @(#) dbgtool.c Version 1.2 (4/22/92). copyright venue & Fuji Xerox  */
static char *id = "@(#) dbgtool.c	1.2 4/22/92	(venue & Fuji Xerox)";
...
extern int URaid_ArrMAXIndex;
	...
	/************************************************/
	/*						*/
	/*     Screen-full handling for BT BT!, etc.	*/
	/*						*/
	/************************************************/
int BT_lines;
int BT_temp;
...
#ifdef DOS
	/* DOS has a 25-line screen, and getchar() discards ESC
	   for some reason. */
#define BTMAXLINE 24
#define BT_morep if(++BT_lines>BTMAXLINE){				\
			 printf("Press Return(to quit Esc instead):");	\
			 BT_temp=getch() ;				\
			 fflush(stdin);					\
			 BT_lines=0; 					\
