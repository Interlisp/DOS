/* @(#) XInit.c Version 1.5 (4/27/92). copyright venue & Fuji Xerox  */
static char *id = "@(#) XInit.c	1.5 4/27/92	(venue & Fuji Xerox)";
...
X_init( dsp, lispbitmap, width_hint, height_hint, depth_hint )
     DspInterface dsp;
     char *lispbitmap;
     int width_hint, height_hint, depth_hint;
{
  ...
  dsp->UpperLeftX = LispDisplayRequestedX;
  dsp->UpperLeftY = LispDisplayRequestedY;

  /* Set the width and height of the display.  */
  if ( height_hint == 0 ){
    dsp->displayheight = HeightOfScreen( Xscreen ) -
      OUTER_SB_WIDTH(dsp); /* In the default case,
			      adjust for scroll gadgets*/
  }
  else dsp->displayheight = bound(WIN_MIN_HEIGHT,
				  height_hint,
				  WIN_MAX_HEIGHT);

  if ( width_hint == 0 ){
    dsp->displaywidth = WidthOfScreen( Xscreen ) -
      OUTER_SB_WIDTH(dsp); /* In the default case,
			      adjust for scroll gadgets*/
  }
  else dsp->displaywidth = bound(WIN_MIN_WIDTH,
				  width_hint,
				  WIN_MAX_WIDTH);
  ...
  /************************************************************/
  dsp->displaywidth = ((dsp->displaywidth+31) >> 5) << 5;

  dsp->enter_display = (PFV)Open_Display;
  dsp->exit_display = (PFV)lisp_Xexit;

  dsp->bitblt_to_screen = (PFUL)clipping_Xbitblt;

  dsp->before_raid = (PFV)Xevent_before_raid;
  dsp->after_raid = (PFV)Xevent_after_raid;

  dsp->BitGravity = NorthWestGravity;

  dsp->cleardisplay = (PFUL)GenericReturnT;
  dsp->set_color_map_entry = (PFUL)GenericReturnT;

  /* Set the geometry of the Vissible (Lisp) window. */
  dsp->VissibleWidth = bound(OUTER_SB_WIDTH(dsp),
			     LispWindowRequestedWidth,
			     min(dsp->displaywidth,
				 (WidthOfScreen( Xscreen ) -
				  OUTER_SB_WIDTH(dsp))));
  dsp->VissibleHeight = bound(OUTER_SB_WIDTH(dsp),
			       LispWindowRequestedHeight,
			       min(dsp->displayheight,
				   (HeightOfScreen( Xscreen ) -
				    OUTER_SB_WIDTH(dsp))));

  /* Initialize the screen image structure. */
  dsp->ScreenBitmap.width   = dsp->displaywidth;
  dsp->ScreenBitmap.height  = dsp->displayheight; 
  ...
  dsp->ScreenBitmap.data =
    (char *)(lispbitmap?Addr68k_from_LADDR((LispPTR)lispbitmap):0);
  ...
  case 8:			/* Color Screen */
    ...
    dsp->ScreenBitmap.bytes_per_line = (dsp->displaywidth);
  ...
  default:			/* B/W Screen */
    ...
    dsp->ScreenBitmap.bytes_per_line = ((dsp->displaywidth+(BITSPER_DLWORD-1))
					/BITSPER_DLWORD) * (BITSPER_DLWORD/8);
    break;
  }

  /* Return the lispbitmap if we were successfull. If we had to */
  /* default the bitmap we return 1. */
  if (lispbitmap) {
    return( lispbitmap );
  } else {
    return ( 1 );
