/* @(#) dsk.c Version 2.64 (4/23/92). copyright venue & Fuji Xerox  */
static char *id = "@(#) dsk.c	2.64 4/23/92	(venue & Fuji Xerox)";
...
#include <dos.h>	/* find_t structure for directory search */
#include <sys/stat.h>
#include <time.h>
#include <io.h>
#define index strchr
	...
	register LispPTR	*args;
{
	...
	if (strcmp(host, "DSK") == 0) dskp = 1;
	else if (strcmp(host, "UNIX") == 0) dskp = 0;
	else return(NIL);
	...
	register LispPTR	*args;
{
	 ...
	 * convert a version field.
	 */
	dskp ? unixpathname(lfname, file, 1, 0, drive, &extlen, rawname) : unixpathname(lfname, file, 0, 0, drive, &extlen, rawname);
	...
	if (dskp) {
		...
		for (; rval == 0;
		     S_TOUT(rval = _dos_findnext(&dirp))) {
	...
	register LispPTR	*args;
{
	...
	char		name[MAXNAMLEN];
	char		ver[VERSIONLEN]
			, drive[1], rawname[MAXNAMLEN];
    int extlen;	/* len of extension, for making DOS backup filename */
	...
	if (unixpathname(lfname, file, 1, 0
#ifdef DOS
			 , drive, &extlen, rawname
#endif /* DOS */
			 ) == 0) return(NIL);
	...
	register LispPTR	*args;
{
	...
	register FileName *varray;
	int extlen, extlen2;
    char rawname[MAXNAMLEN], rawname2[MAXNAMLEN];
	...
	unixpathname(fbuf, src, 1, 0, drive, &extlen, rawname);
#else /* DOS */
	...
	unixpathname(fbuf, src, 1, 0, drive2, &extlen2, rawname2);
#else /* DOS */
	unixpathname(fbuf, src, 1, 0);
#endif /* DOS */
	...
	register LispPTR	*args;
{
	...
	if (*drive <= 'Z') rval = _chdrive(*drive - ('A'-1));
	else rval = _chdrive(*drive-('a'-1));
	if (rval != 0) return(NIL);
	}
#endif /* DOS */
	return(ATOM_T);
...
COM_getfreeblock(args)
  register LispPTR *args;
  {
    ...
    struct diskfree_t sfsbuf;	/* DOS free-space structure */
  ...
  LispStringLength(args[0], rval, dskp);
...
#endif /* AIXPS2 | HPUX SYSVONLY RS6000 DEC3100 */
	...
	*buf = sfsbuf.avail_clusters * sfsbuf.sectors_per_cluster
		* sfsbuf.bytes_per_sector;
      }
    else
      {
	if (_dos_getdiskfree(0, &sfsbuf))
	  return(NIL); /* call failed, so name is invalid */
 ...
 *
 * Argument:	file   UNIX file name in the UNIX format.  It must be an
 *				absolute path.
 *		dir	The place where unpacked directory will be stored.
 *		name	The place where unpacked "root file name" will be
 *			stored. "Root file name" contains the file name
 *			and the extension.
 *		ver	The place where unpacked version will be stored.
 *		checkp	If 1, whether the version field contains only the
 *			numeric characters or not will be checked.
 *
 * Value:	If succeed, returns 1, otherwise 0.
 *
 * Side Effect:	dir, name, and  ver will be replaced with the
 *		unpacked corresponding fields.
 *
 * Description:
 *
 * Unpack the specified UNIX full pathname to three components.
 * If the file does not include a version field, NULL string
 * will be stored as a version.  unpack_filename assumes the
 * pathname passed is a file, not a directory.
 *
 */
	...
	register int	checkp;
{
...
#ifdef DOS
    if ((cp =(char *) max((unsigned long) rindex(file, DIRSEP),
			  (unsigned long) rindex(file,UNIXDIRSEP))) == 0)
      return(0);

    if (file[1] == DRIVESEP)
      { /* There's a drive spec; copy it and ignore it from here on. */
	*dir++ = *file++;
	*dir++ = *file++;
      }
#else /* DOS */
	if ((cp =  rindex(file,UNIXDIRSEP)) == 0) return(0);
#endif /* DOS */
	...
	*(dir + 1) = '\0';
      ...
      { /* not just root dir; copy directory name. */
	...
	register char	*path;
{
	...
	*dir = '\0';	/*
			 ...
			 * as directory.
			 */
	cp = path + 1;	/* Skip the first directory delimiter */
#ifdef DOS
      }
#endif /* DOS */
	...
	while (*cp) {
		 ...
		 */
		sp = name;
		for (c= *cp++, *sp++ = c; c != DIRSEP && c != '\0' && c != '/';c= *cp++, *sp++ = c) {}
		if ((c == DIRSEP)||(c == '/')) {
	...
	register char	*dir;
{
    ...
    if (DRIVESEP == *(cp+1))
      {
	*dp++ = *cp++;	/* copy the drive letter & colon */
	*dp++ = *cp++;
      }
#endif /* DOS */
	*dp++ = DIRSEP;		/* For a root directory. */
	...
	for (;;) {
		switch (*cp) {
		      ...
		      case DIRSEP:
#endif
		      case '/':  /* even in DOS, for compatibility */
		      case '\0':
			*dp = '\0';
			/*
...
/************************************************************************/
/*									*/
/*		    F i n d H i g h e s t V e r s i o n			*/
/*									*/
/*	Find the highest-version entry in VARRAY.  VARRAY must		*/
/*	include at least 1 versioned file, i.e., VARRAY must satisfy	*/
/*	(!NoFileP(varray) && !OnlyVersionlessP(varray)).		*/
/*									*/
/*	varray	The version array; must include >= 1 versioned file.	*/
/*	mentry	Place to put the VARRAY entry corresponding to the	*/
/*		highest-versioned file.					*/
/*	max_no	Place to put the version number of that file.		*/
/*									*/
/*	Sets *max_no & *mentry to the file & version.			*/
/*									*/
/************************************************************************/
...
#ifdef DOS
get_version_array(dir, file, varray, cache, extlen)
  register char		*dir;
  register char		*file;
  register FileName	*varray;
  register CurrentVArray	*cache;
  int extlen;	/* len of the file's extension */
#else
get_version_array(dir, file, varray, cache)
   ...
   */
    ...
    strcpy(old_file, lcased_file);
    if(extlen == 0) strcat(old_file, ".%"); /* no extension, add one */
    else if (extlen < 3) strcat(old_file,"%"); /* short exten; add % */
    else old_file[strlen(old_file)-1] = '%'; /* full exten; replace last char */
