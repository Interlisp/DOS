...
*  and the Assembly '_Life()' portion, which plays each game.
*
...
*  routine also calls an Assembly routine, '_Delay()', which invokes a BIOS
...
BYTE  _Life(int color, VIDEO *buf1, VIDEO *buf2, BYTE *cmpBuf);

/******************************************************************************
*  Global variables (also referenced in the Assembly code)
******************************************************************************/
int                  _numCols;         /* # of cols & rows (from video config*/
int                  _numRows;
...
*        Calls the Assembly routine '_Life()' to play the game until it ends or
*           a key is pressed.
...
******************************************************************************/
   ...
   _numCols = config.numtextcols;
   _numRows = config.numtextrows;
   buf1 = (VIDEO *)malloc(_numCols * _numRows * sizeof(VIDEO));
   buf2 = (VIDEO *)malloc(_numCols * _numRows * sizeof(VIDEO));
   cmpBuf = (BYTE *)malloc((_numCols+2) * (_numRows+2));
...
******************************************************************************/
   do
   {
      ...
      bufPtr = buf1;
      for (row = 0; row < _numRows; row++) /* fill buffer with 1st generation*/
      {
         for (col = 0; col < _numCols; col++, bufPtr++)
         {
            if ((rand() & 3) == 3)
               bufPtr->attrib = color;
            else
      ...
      memcpy(PHYS_VID, buf1, _numCols * _numRows * 2);/* copy 1st gen to scrn*/


      quit = _Life(color, buf1, buf2, cmpBuf);
      if (! quit)                      /* keep final generation on screen    */
         _Delay(4000);                 /*    for a while (4000msecs = 4secs) */
   }
   while (! quit);
...
******************************************************************************/
   vidPtr = (VIDEO *)PHYS_VID;
   bufPtr = cmpBuf + ((_numRows + 1) * (_numCols + 2));
   for (col = 0; col < _numCols; col++)
      *bufPtr++ = vidPtr++->attrib;
...
******************************************************************************/
   vidPtr = (VIDEO *)PHYS_VID + ((_numRows - 1) * _numCols);
...
******************************************************************************/
   vidPtr = (VIDEO *)PHYS_VID + (_numCols - 1);
   bufPtr = cmpBuf + (_numCols + 2);    /* skip 0th (missing) row of cmpBuf  */
   for (row = 0; row < _numRows; row++, bufPtr += _numCols+2,
        vidPtr += _numCols)
      *bufPtr = vidPtr->attrib;
...
******************************************************************************/
   vidPtr = (VIDEO *)PHYS_VID;
   bufPtr = cmpBuf + (2*(_numCols + 2) - 1);    /* skip 0th (missing) row    */
   for (row = 0; row < _numRows; row++, bufPtr += _numCols+2,
        vidPtr += _numCols)
      *bufPtr = vidPtr->attrib;
...
******************************************************************************/
   vidPtr = (VIDEO *)PHYS_VID + (_numCols*_numRows) - 1;
   bufPtr = cmpBuf;
   *bufPtr = vidPtr->attrib;
...
******************************************************************************/
   vidPtr = (VIDEO *)PHYS_VID + _numCols - 1;
   bufPtr += (_numRows * (_numCols + 2)) + 1;
   *bufPtr = vidPtr->attrib;

/******************************************************************************
*   Copy screen's top,left corner to cmpBuf's bottom,right                   
******************************************************************************/
   bufPtr += _numCols + 1;             /* advance to bottom, right element   */
   vidPtr = (VIDEO *)PHYS_VID;
   *bufPtr = vidPtr->attrib;
